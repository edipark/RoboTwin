...
class RoboTwinDataset(Dataset):
    ...
    def _decode_image(self, jpeg_bytes) -> np.ndarray:
        """JPEG bytes → RGB float32 [H, W, 3] resized to image_size."""
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # ── State ─────────────────────────────────────────────────────────
        state = ep["qpos"][t].copy()                   # [actual_state_dim]

        # Zero-pad if actual state dim < model's action_dim
        actual_dim = state.shape[0]
        if actual_dim < self.action_dim:
            state = np.pad(state, (0, self.action_dim - actual_dim))
        ...
        if self.norm_stats is not None and "state" in self.norm_stats:
            ...
            mean = ns.mean
            std  = ns.std
            if len(mean) < self.action_dim:
                mean = np.pad(mean, (0, self.action_dim - len(mean)))
                std  = np.pad(std,  (0, self.action_dim - len(std)), constant_values=1.0)
        ...
        chunk_len = end - t
        raw_action = ep["actions"][t:end]  # [chunk_len, actual_action_dim]
        ...
        # Normalize action (norm_stats key is "actions" plural — matches upstream openpi)
        if self.norm_stats is not None and "actions" in self.norm_stats:
            ...
            if len(mean) < self.action_dim:
                mean = np.pad(mean, (0, self.action_dim - len(mean)))
                std  = np.pad(std,  (0, self.action_dim - len(std)), constant_values=1.0)
            action_chunk = (action_chunk - mean) / (std + 1e-8)
        ...
        # ── Tokenize (Pi05 mode: embeds state_norm into text prompt) ──────
        ...
        return {
            ...
            "cam_right_wrist":        cam_right_wrist,
            "cam_left_wrist":         cam_left_wrist,
            # State: [action_dim] float32 (normalized, clipped to [-1, 1])
